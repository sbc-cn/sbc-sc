...
(define-map minted-tsbc-by-user
    {btc-address: (buff 40)}
    {total-minted-tsbc: uint} 
)

;; URI for TSBC token
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://abc/tsbc.json"))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (ft-transfer? tsbc amount from to)
  )
)
...
(define-read-only (get-balance (user principal))
  (ok (ft-get-balance tsbc user))
)

;; the current total supply (which does not need to be a constant)
(define-read-only (get-total-supply)
  (ok (ft-get-supply tsbc))
)
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? tsbc amount recipient)
  )
)

;; Registers a new BTC address and initialize its total minted tsbc.
(define-public (regist-tsbc-user (btc-address (buff 40)))
  (begin
    (asserts! (is-caller-contract-owner) (err ERR_CONTRACT_OWNER_ONLY))
    (asserts! (is-eq (map-get? minted-tsbc-by-user {btc-address: btc-address}) none) (err ERR_USER_ALREADY_REGISTERED))
    (map-insert minted-tsbc-by-user {btc-address: btc-address} {total-minted-tsbc: u0})
    (ok true)
  )
)
...
(define-public (mint-and-record (amount uint) (recipient principal) (btc-block uint) (btc-address (buff 40)))
  (begin
    (asserts! (is-caller-contract-owner) (err ERR_CONTRACT_OWNER_ONLY))
    (asserts! (not (is-eq (map-get? minted-tsbc-by-user {btc-address: btc-address}) none)) (err ERR_USER_NOT_REGISTERED))
    (try! (ft-mint? tsbc amount recipient))
    (map-set minted-block-info {block: btc-block} {btc-address: btc-address, btc-amount: amount, tsbc-amount: amount})
    (let ((current-btc 
      (get total-minted-tsbc (unwrap! (map-get? minted-tsbc-by-user {btc-address: btc-address}) (err ERR_USER_NOT_REGISTERED)))))
      (map-set minted-tsbc-by-user {btc-address: btc-address} {total-minted-tsbc: (+ current-btc amount)})                    
    )
    (ok true)
  )
)